...
def check(l):
    ...
    for i in range(3):
        if(l[i]== l[i+3] and l[i]==l[i+6] and l[i] not in numbers):return l[i]
        if(l[i*3]== l[i*3+1] and l[i]==l[i*3+2]and l[i] not in numbers):return l[i]
    if(((l[0]==l[4] and l[4]==l[8]) or (l[2]==l[4] and l[4]==l[6]))and l[4] not in numbers):return l[4]
    draw = 1
    for i in range(9):
        if(l[i]==i+1):draw = 0
    if(draw):return -1
...
def valid(l, p):
    return p<9 and p>=0 and l[p] in tuple(range(1, 10))
...
print(f"{players[0]} spiller som {mark[0]}, {players[1]} spiller som {mark[1]}")
# sleep(3)
positions()
# sleep(3)
...
while 1:
    while not check(l):
        ...
        val = 1
        while(val):
            p = input(f"{players[turn%2]} ({mark[turn%2]}) sin tur: \nHvor vil du legge briken din? (1-9)")
            try:
                p = int(p)-1
                val = 0
            except:
    ...
    else: print(f"Gratulerer {players[(mark.index(check(l)))%2]}")
